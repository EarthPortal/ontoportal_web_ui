  ...
  def external_tools_list
    tools = Rails.cache.fetch('external_tools_list', expires_in: 1.hour) do
      LinkedData::Client::Models::ExternalTool.all
        ...
        .map { |tool| { id: tool.id.to_s, name: tool.name, label: tool.title, url: tool.homepage.to_s, type: tool.externalToolType.to_s } }
    end
    tools || []
  end

  def external_editors_list
    external_tools_list.select { |tool| tool[:type] == 'EDITOR' }
  end
